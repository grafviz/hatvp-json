...
keyWord=sys.argv[1]
filename= sys.argv[2] if len(sys.argv)>2 else 'declarations-'+today+'.json'

filename2= sys.argv[3] if len(sys.argv)>3 else 'dernieres-declarations-'+today+'.json'
...
with open('resultats-'+filename+'-search-'+keyWord+".csv", "w+") as f:
	for k in results.keys():
		print(k)
		f.write(k+','+results[k]+'\n')

with open('resultats-'+filename2+'-search-'+keyWord+".csv", "w+") as f:
